...
private:
	...
	{
		return fMIKEYState.keyData();
	}

	u_int8_t const *masterKey() const
	{
		return &masterKeyPlusSalt()[0];
	}
	...
	{
		return fMIKEYState.encryptSRTP();
	}
	Boolean weEncryptSRTCP() const
	{
		return fMIKEYState.encryptSRTCP();
	}
	Boolean weAuthenticate() const
	{
		return fMIKEYState.useAuthentication();
	}
	u_int32_t MKI() const
	{
		return fMIKEYState.MKI();
	}
